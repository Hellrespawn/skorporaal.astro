...
async function main(): Promise<void> {
  ...
  try {
    PRINTER.heading('Running pre-deploy tasks:');
    await rename404();

    PRINTER.done();

    PRINTER.heading('Attempting to upload files...');
    await checkSitePath();
    ...
    client.close();
    PRINTER.done();
    PRINTER.heading('Running post-deploy tasks:');
    await undo404();
